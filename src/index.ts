/**
 * PROTOTYPE — dsh-voice-mini v0.2. Voice feedback for DeepSeek Harness.
 *
 * Two reply kinds (per the design):
 * - 结果回复 (result): full narration of an assistant message, or the
 *   agent's own `speak` tool call. Existing logic, now serialized through
 *   a queue so utterances never overlap.
 * - 进行中回复 (status): short spoken notices on lifecycle events the
 *   human should hear while away — `approval/asked`, `approval/decided`,
 *   `ask_user_question` tool call, `turn/start`, `turn/end`, `todo/write`.
 *
 * Each utterance runs: ① earcon (提示音) → ② speech, in that order, on a
 * single-worker queue (no overlaps, guarantees chime-then-speech).
 *
 * Half-host, half-client (a modal settings window): the header 🔊 icon
 * (now an SVG) opens it; settings sit on top, info below.
 *
 * Backends: edge (cloud, zh+en) · kokoro (local 0.08B, en only) · say (macOS)
 * · fake (wiring tests). rc.6 note: custom session events are NOT persisted
 * (they poison history load), so this prototype keeps everything in memory.
 *
 * @module dsh-voice-mini
 */
...
interface MiniConfig {
  ...
  announceApproval: boolean;
  announceQuestion: boolean;
  announceTurnStart: boolean;
  announceTurnEnd: boolean;
  announceTodo: boolean;
 ...
 */
const PRESETS: Record<string, Partial<MiniConfig>> = {
...
const STATUS_KEYS: ReadonlyArray<keyof MiniConfig> = [
  'statusEnabled', 'announceApproval', 'announceQuestion', 'announceTurnStart', 'announceTurnEnd', 'announceTodo',
];

/** One queued utterance. `chime` is true only for the first item in a burst
 * (queue idle→busy) so a run of back-to-back notices rings the earcon once.
...
interface QueueItem {
  ...
  synthPromise?: Promise<PrefetchedSynth>;
  synthStarted?: boolean;
}

/** A synthesized clip + the voice that actually produced it (may differ from
 * the chosen voice after a fallback retry). */
interface PrefetchedSynth {
  path: string;
  ms: number;
  usedVoice: string;
}

/** A spoken record shown in the info panel. */
...
interface MiniState {
  ...
  lastError?: string;
  lastUrl?: string;
  /** Human-readable label of the voice the last utterance used. */
  lastVoice?: string;
}
 ...
 */
const PERSONA_BY_PRESET: Record<string, string> = {
  instant: [
...
function scrubForSpeech(text: string, codeOmitted = '（代码略）'): string {
  return text
...
function extForBackend(backend: string): string {
...
function cap(text: string, n = 300, suffix = '…（后略）'): string {
 ...
 */
function desktopRendererHeader(ctx: Context): { name: string; value: string } | undefined {
  const access = ctx.get('desktopBrowserAccess') as
    | { rendererHeader?: { name?: unknown; value?: unknown } }
    | undefined;
  const name = access?.rendererHeader?.name;
  const value = access?.rendererHeader?.value;
  if (typeof name !== 'string' || typeof value !== 'string' || name === '' || value === '') return undefined;
  return { name, value };
}
...
export function apply(ctx: Context, rawConfig: unknown): void {
  const entry = { ...Config(rawConfig ?? {}), ...(rawConfig as object) } as MiniConfig;
  const state: MiniState = { readReplies: entry.readReplies };
  const recent: SpokenRecord[] = []; // ring buffer, cap 30

  // ── metrics: per-call performance + token tracking ────────────────────────
  interface CallMetric {
    at: number; kind: string; sessionId?: string;
    llmMs?: number; ttsMs?: number; playMs?: number; totalMs?: number;
    inputTokens?: number; outputTokens?: number; textLen?: number;
    success: boolean; error?: string;
  }
  const metrics: CallMetric[] = []; // ring buffer, cap 100
  // Aggregates computed on demand from `metrics`.
  const pushMetric = (m: CallMetric): void => {
    metrics.push(m);
    if (metrics.length > 100) metrics.splice(0, metrics.length - 100);
  };
  // System stats sampled periodically.
  let sysStats: { cpuUser: number; cpuSystem: number; rssMb: number; heapMb: number; audioMb: number } = {
    cpuUser: 0, cpuSystem: 0, rssMb: 0, heapMb: 0, audioMb: 0,
  };
  let lastCpu = process.cpuUsage();
  let lastCpuTime = Date.now();
  const sampleSystem = async (): Promise<void> => {
    const now = Date.now();
    ...
    try {
      ...
      for (const f of readdirSync(dir)) {
        try { audioBytes += statSync(join(dir, f)).size; } catch { /* */ }
      }
    } catch { /* dir missing */ }
    sysStats = {
  ...
  const sysTimer = setInterval(() => { void sampleSystem(); }, 30_000); sysTimer.unref();
  ...
  const voiceOverride = new Map<string, { voice: string; rateJitter: number }>();
  ...
  ctx.inject(['llm'], (llmCtx) => {
    llmService = llmCtx.get('llm') as typeof llmService;
  ...
  let titleService: { get(target: unknown): { title?: string } | undefined } | undefined;
  ctx.inject(['sessionTitle'], (titleCtx) => {
    titleService = titleCtx.get('sessionTitle') as typeof titleService;
    ctx.logger.warn('dsh-voice-mini: sessionTitle service attached');
  });
  const titleFor = (session: unknown): string | undefined => {
    try {
   ...
   */
  async function summarizeReply(replyText: string, sid?: string): Promise<string | null> {
    if (llmService === undefined || replyText.trim() === '') return null;
    ...
    try {
      ...
      if (!provider) {
        ...
        if (adapters instanceof Map && adapters.size > 0) provider = [...adapters.keys()][0];
      }
      if (!provider) { ctx.logger.warn('dsh-voice-mini: no LLM provider, verbalizer disabled'); return null; }
      let model = config.summarizeModel;
      if (model === '') {
        try { const models = await llmService.listModels?.(provider); if (Array.isArray(models) && models.length > 0) { model = models[0]!.id; ctx.logger.warn(`dsh-voice-mini: auto-detected provider=${provider} model=${model}`); } } catch { /* */ }
      }
      if (!model) { ctx.logger.warn(`dsh-voice-mini: no model for provider ${provider}`); return null; }
      ...
      const buildOpts = (withEffort: boolean): Record<string, unknown> => ({
      ...
      const streamOnce = async (opts: Record<string, unknown>): Promise<boolean> => {
        let sawText = false;
        ...
        try {
          ...
          for await (const chunk of llmService!.stream(opts)) {
            const ct = String((chunk as any).type);
            if (ct === 'text-delta') { sawText = true; accumulated += String((chunk as any).text ?? ''); flushSentences(false); }
            else if (ct === 'finish') { finishReason = (chunk as any).reason; ctx.logger.warn(`dsh-voice-mini: verbalizer finish reason=${JSON.stringify(finishReason)}`); }
          ...
          for await (const chunk of llmService!.stream(opts)) {
            const ct = String((chunk as any).type);
            if (ct === 'text-delta') { sawText = true; accumulated += String((chunk as any).text ?? ''); flushSentences(false); }
            if (ct === 'finish') finishReason = (chunk as any).reason;
          }
        }
        ...
        if (!sawText && msg.includes('does not support reasoning effort')) return false;
        return true;
      };
      ...
      // Token usage.
      const usage = (assembler as any)?._usage;
      ...
      if (controller.signal.aborted) ctx.logger.warn('dsh-voice-mini: verbalizer timed out, fallback to template');
      else ctx.logger.warn('dsh-voice-mini: verbalizer failed', error);
      return null;
    } finally {
      clearTimeout(timer);
    }
  ...
  const compositionKeys = Object.keys((rawConfig ?? {}) as object);
  let current: () => MiniConfig = () => entry;
  let settingsSource: (() => MiniConfig) | undefined;
  ...
  const loadPersistedConfig = (): Partial<MiniConfig> => {
    if (NO_PERSIST) return {};
    try {
      const obj = JSON.parse(readFileSync(persistedConfigPath, 'utf8'));
      return typeof obj === 'object' && obj !== null ? obj as Partial<MiniConfig> : {};
    } catch { return {}; }
  };
  const savePersistedConfig = (cfg: Partial<MiniConfig>): void => {
    if (NO_PERSIST) return;
    try { mkdirSync(persistedConfigDir, { recursive: true }); writeFileSync(persistedConfigPath, JSON.stringify(cfg, null, 2)); }
    catch (e) { ctx.logger.warn('dsh-voice-mini: failed to persist config', e); }
  };
  let persistedConfig = loadPersistedConfig();

  const settingsDelta = (): Partial<MiniConfig> => {
    if (settingsSource === undefined) return {};
    const values = settingsSource();
    const delta: Partial<MiniConfig> = {};
    for (const [k, v] of Object.entries(values)) {
      if (v !== (entry as any)[k]) (delta as any)[k] = v;
    }
    return delta;
  };

  const resolve = (): MiniConfig => {
    const delta = settingsDelta();
    const preset = String(persistedConfig.preset ?? delta.preset ?? entry.preset);
    const base: MiniConfig = { ...entry, ...(PRESETS[preset] ?? {}) };
    // Composition-explicit keys outrank the gear, so a hand-written
    // `announceTurnEnd: true` survives a switch to 少量沟通.
    for (const k of compositionKeys) (base as any)[k] = (rawConfig as any)[k];
    return { ...base, ...delta, ...persistedConfig };
  };
  current = resolve;
  installSettingsSection(ctx, 'dsh-voice-mini', Config, entry, {
    setSource: (source) => {
      settingsSource = () => ({ ...entry, ...(source() as object) }) as MiniConfig;
      current = resolve;
    },
    onChange: () => {
      current = resolve;
  ...
  const parseConfigPatch = (body: any): Partial<MiniConfig> => {
    const patch: Partial<MiniConfig> = {};
    if (typeof body?.preset === 'string' && (PRESET_KEYS as readonly string[]).includes(body.preset)) patch.preset = body.preset;
    if (typeof body?.backend === 'string' && ['edge', 'kokoro', 'say', 'fake'].includes(body.backend)) patch.backend = body.backend;
    if (typeof body?.voiceMode === 'string' && ['per-session', 'fixed'].includes(body.voiceMode)) patch.voiceMode = body.voiceMode;
    if (Array.isArray(body?.voicePalette)) {
      patch.voicePalette = body.voicePalette.filter((v: unknown): v is string => typeof v === 'string' && v.trim() !== '').map((v: string) => v.trim());
    }
    if (typeof body?.voice === 'string' && body.voice.trim() !== '') patch.voice = body.voice.trim();
    if (typeof body?.readReplies === 'boolean') patch.readReplies = body.readReplies;
    ...
    if (typeof body?.chimeEnabled === 'boolean') patch.chimeEnabled = body.chimeEnabled;
    if (typeof body?.chimeSpeech === 'string' && ['glass', 'ding', 'ping', 'soft', 'none'].includes(body.chimeSpeech)) patch.chimeSpeech = body.chimeSpeech;
    if (typeof body?.chimeStatus === 'string' && ['glass', 'ding', 'ping', 'soft', 'none'].includes(body.chimeStatus)) patch.chimeStatus = body.chimeStatus;
    ...
    for (const k of [...STATUS_KEYS, 'announceToolCall'] as ReadonlyArray<keyof MiniConfig>) {
      if (typeof body?.[k] === 'boolean') (patch as any)[k] = body[k];
    }
    ...
    if (typeof body?.summarizeResult === 'boolean') patch.summarizeResult = body.summarizeResult;
    if (typeof body?.summarizeProvider === 'string') patch.summarizeProvider = body.summarizeProvider.slice(0, 100);
    if (typeof body?.summarizeModel === 'string') patch.summarizeModel = body.summarizeModel.slice(0, 100);
    ...
    if (typeof body?.jarvisLinked === 'boolean') patch.jarvisLinked = body.jarvisLinked;
  ...
  const snapshot = () => {
    ...
    return {
      ...
      /** The resolved Jarvis voice: empty config → locale-matched default
      ...
      sessionVoice: lastSessionId ? (voiceOverride.has(lastSessionId)
        ...
        : sessionVoiceFor(lastSessionId, { mode: config.voiceMode, voice: config.voice, palette: config.voicePalette, backend: config.backend, locale: config.locale }).voice
        ).replace(/^zh-CN-|Neural$/g, '') : undefined,
  ...
  async function prefetchSynth(item: QueueItem): Promise<PrefetchedSynth> {
    const config = current();
    ...
    const attempt = async (voice: string): Promise<{ path: string; ms: number }> => {
      ...
      if (size === 0) throw new Error(`${backend.id}: synthesis produced an empty file`);
      ...
      if (dur > 0 && dur < minDur) throw new Error(`${backend.id}: synthesis too short (${dur.toFixed(1)}s for ${textLen} chars — partial file)`);
      return result;
    };
    ...
    try {
      ...
      // preserved (no mid-turn voice flip). A short backoff lets the
      // server/network blip clear; retrying IMMEDIATELY re-hits the same blip
      // (this was the root cause of "no sound": all three same-backend attempts
      // failed in quick succession against a momentary edge outage).
      ...
      try {
        ...
        // Same voice failed twice → try the configured default voice (a voice
        // Microsoft no longer serves, or a different timbre). Still edge-tts.
        ...
        try {
          ...
          // edge-tts is having a bad moment (server down / network) — every
          // edge attempt failed. LAST RESORT: macOS `say` (offline, no network,
          // always available on this platform) so the user hears SOMETHING
          // instead of silence. Different voice quality, but silence is
          // unacceptable for a voice plugin. say's zh voice Tingting reads
          // digits natively, so no digitsToZh is applied here.
          const sayVoice = config.locale === 'en' ? 'Samantha' : 'Tingting';
          ctx.logger.warn(`dsh-voice-mini: edge exhausted (3×), falling back to macOS say (${sayVoice}) after "${third instanceof Error ? third.message : String(third)}"`);
          const sayBackend = makeBackend('say');
          const sayFile = await freshAudioFile(audioDir, extForBackend('say'));
          const sayResult = await sayBackend.synthesize(item.text, sayFile, sayVoice, {
            ratePct: config.ratePct, volumePct: config.volumePct,
          });
          const saySize = existsSync(sayResult.path) ? statSync(sayResult.path).size : 0;
          if (saySize === 0) throw new Error(`say: produced an empty file (last resort failed)`);
          return { path: sayResult.path, ms: sayResult.ms, usedVoice: `[say] ${sayVoice}` };
        }
      }
    }
  }
  ...
  ctx.inject(['systemPrompt'], (promptCtx) => {
    const systemPrompt = promptCtx.get('systemPrompt') as
    ...
    systemPrompt?.section?.({
      ...
      text: () => {
        if (current().readReplies) return PERSONA_NARRATION;
  ...
  ctx.tools.register(defineTool({
    name: 'speak',
    description:
      'Speak a short message aloud on the user\'s speaker (a 提示音 plays first). ' +
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown) => {
        const v = value as { status: string; ms: number; error?: string };
    ...
    async execute(args: { text: string }, exec: { agent?: { session?: { id?: string } } }) {
  ...
  ctx.on('session/event' as any, (session: { id?: string } | undefined, event: { type?: string; data?: any }) => {
    ...
    switch (type) {
      ...
      case 'approval/asked': {
        ...
        if (!config.statusEnabled || !config.announceApproval) return;
      ...
      case 'tool/call': {
        ...
        if (data?.name === 'ask_user_question') {
          ...
          if (!config.statusEnabled || !config.announceQuestion) return;
          if (!config.statusSpeech) { void enqueue('chime', '', sid); return; }
      ...
      case 'todo/write': {
        if (!config.statusEnabled || !config.announceTodo) return;
        if (!config.statusSpeech) { void enqueue('chime', '', sid); return; }
        const todos: unknown[] = Array.isArray(data?.todos) ? data.todos : [];
        const done = todos.filter((it: any) => it?.status === 'completed').length;
  ...
  ctx.inject(['webServer'], (serverCtx) => {
    const webServer = serverCtx.get('webServer') as
      | {
        host?: string;
        port?: number;
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (url === '/session-toggle' && req.method === 'POST') {
            ...
            if (!sessionId) return sendJson(res, 400, { error: 'no session id' });
          ...
          if (url === '/voice/reroll' && req.method === 'POST') {
            const body = JSON.parse((await readBody(req)) || '{}');
            const sessionId = typeof body.sessionId === 'string' ? body.sessionId : lastSessionId;
            if (!sessionId) return sendJson(res, 400, { ok: false, error: 'no session id' });
            const config = current();
            const palette = (config.voicePalette.length > 0 ? config.voicePalette : defaultPalette(config.backend, config.locale))
              .filter((v) => typeof v === 'string' && v !== '');
            if (palette.length === 0) return sendJson(res, 200, { ok: false, error: 'empty palette' });
          ...
          if (url === '/metrics' && req.method === 'GET') {
            ...
            for (const m of calls) byKind[m.kind] = (byKind[m.kind] ?? 0) + 1;
            return sendJson(res, 200, {
              calls,
              sys: sysStats,
              summary: {
