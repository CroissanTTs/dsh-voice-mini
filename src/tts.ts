...
export interface TtsBackend {
  readonly id: string;
  synthesize(text: string, outFile: string, voice?: string, delivery?: Delivery): Promise<SynthResult>;
}

/**
 * How to deliver a line — the "声音" dimension of a communication preset.
 * One numeric knob per dimension, mapped per backend (edge takes SSML-style
 * percentages, kokoro a speed multiplier, `say` words-per-minute).
 */
export interface Delivery {
  /** Speech-rate delta in percent: +18 faster, -5 slower. */
  readonly ratePct: number;
  /** Volume delta in percent: 0 normal, -30 quieter (applied at playback). */
  readonly volumePct: number;
}

export const DEFAULT_DELIVERY: Delivery = { ratePct: 0, volumePct: 0 };
...
const DEFAULT_PALETTES: Record<string, readonly string[]> = {
...
export function defaultPalette(backend: string, locale?: string): readonly string[] {
...
export interface VoiceChoice {
  ...
  /** Extra rate offset on top of the gear's, in percent. */
 ...
 */
export function sessionVoiceFor(
  sessionId: string | undefined,
  options: { mode: string; voice: string; palette?: readonly string[]; backend: string; locale?: string },
): VoiceChoice {
...
export class EdgeBackend implements TtsBackend {
  readonly id = 'edge';
  async synthesize(
    text: string, outFile: string, voice = 'zh-CN-XiaoxiaoNeural', delivery = DEFAULT_DELIVERY,
  ): Promise<SynthResult> {
    const t0 = Date.now();
...
export class KokoroBackend implements TtsBackend {
  readonly id = 'kokoro';
  async synthesize(
    text: string, outFile: string, voice = 'af_heart', delivery = DEFAULT_DELIVERY,
  ): Promise<SynthResult> {
    const t0 = Date.now();
    ...
    if (!existsSync(outFile)) throw new Error('kokoro: child produced no audio (see DSH_VOICE_MINI_DEBUG)');
...
export class SayBackend implements TtsBackend {
  readonly id = 'say';
  async synthesize(
    text: string, outFile: string, voice = 'Tingting', delivery = DEFAULT_DELIVERY,
  ): Promise<SynthResult> {
    const t0 = Date.now();
    ...
    if (code !== 0 || !existsSync(outFile)) throw new Error(`say exited ${code}`);
    return { path: outFile, mime: 'audio/aiff', ms: Date.now() - t0 };
  }
}

/** fake — writes a marker file, no audio. For wiring tests. */
export class FakeBackend implements TtsBackend {
  readonly id = 'fake';
  async synthesize(text: string, outFile: string): Promise<SynthResult> {
    await writeFile(outFile, `FAKE TTS: ${text}\n`, 'utf8');
    return { path: outFile, mime: 'text/plain', ms: 0 };
  }
}

export function makeBackend(id: string): TtsBackend {
  switch (id) {
...
export function play(file: string): void {
  if (process.platform !== 'darwin') return; // prototype: macOS only
...
export function playAndWait(
  file: string,
  options: { timeoutMs?: number; volumePct?: number } = {},
): Promise<void> {
...
function tone(freq: number, seconds: number, decay = 4, attack = 0.004): Float32Array {
...
function chimeWave(sound: string): Float32Array | null {
  switch (sound) {
    case 'ding': {
...
export function chimeFileExt(sound: string): string {
 ...
 */
export async function writeChimeFile(sound: string, audioDir: string): Promise<string | null> {
  if (sound === 'glass') {
    const src = '/System/Library/Sounds/Glass.aiff';
    if (!existsSync(src)) return null; // non-macOS or missing
    await mkdir(audioDir, { recursive: true });
    const file = join(audioDir, 'chime-glass.aiff');
    if (!existsSync(file)) copyFileSync(src, file);
    return file;
  }
  ...
  if (!existsSync(file)) await writeFile(file, encodeWav(wave));
  return file;
}

export function resolveAudioDir(dir: string): string {
  return resolve(dir.replace(/^~/, homedir()));
}

export async function freshAudioFile(dir: string, ext: string): Promise<string> {
 ...
 */
export function audioDurationSec(file: string): Promise<number> {
  return new Promise((resolve) => {
    if (process.platform !== 'darwin') return resolve(0);
